...
function restoreInputs() {
  try { const { url, id } = JSON.parse(atob(location.hash.slice(1))); if (url) urlInput.value=url; if (id) idInput.value=id; } catch {}
}
restoreInputs();
...
function dayMatchesSpec(jsDay, spec) {
  ...
  if (s.includes('-')) {
    const [a,b]=s.split('-'), st=svDay(a), en=svDay(b);
...
function splitTimeParts(timesStr) {
  if (!timesStr) return [];
  return normalizeTimesStr(timesStr).replace(/\s*(?:ll|\|\|)\s*/gi,'\n').replace(/\r/g,'').split('\n').map(p=>p.trim()).filter(Boolean);
}
...
function renderNextShiftBanner(schedule) {
  ...
  function updateBanner(){
    ...
    nextShiftBanner.innerHTML=`<div class="nsb-icon">${isToday?'⏰':'📅'}</div><div class="nsb-body"><div class="nsb-label">${isToday?'Shift today':'Next shift'}</div><div class="nsb-venue">${info?info.name:nextShiftCode}<span style="font-size:.75rem;font-weight:400;color:var(--text-muted);">&nbsp;${nextShiftCode}</span></div><div class="nsb-detail">${dateLabel}${nextShiftTime?' &middot; '+nextShiftTime:''}</div></div><div class="nsb-countdown"><div class="nsb-countdown-value">${countdown}</div><div class="nsb-countdown-label">until start</div></div>`;
  }
  updateBanner();
